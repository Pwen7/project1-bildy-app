...
export const registerUser = async (req, res, next) => {
  try {
    ...
    sendVerificationEmail(email, verificationCode).catch((err) =>
      console.error('⚠️ [Mail] Could not send verification email:', err.message)
    )
...
export const verifyEmail = async (req, res, next) => {
  try {
    ...
    if (user.verificationAttempts <= 0) {
      throw AppError.tooManyRequests('No attempts remaining')
    }
...
export const uploadLogo = async (req, res, next) => {
  try {
    ...
    const user = await User.findById(req.user._id)
    if (!user.company) {
      throw AppError.badRequest('User has no company')
    }

    const logoUrl = await uploadToCloud(req.file, { folder: 'logos' })
...
export const inviteUser = async (req, res, next) => {
  try {
    ...
    if (!inviter.company) {
      throw AppError.badRequest('You must have a company to invite users')
    }
